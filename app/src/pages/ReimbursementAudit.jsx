...
import { IndianRupee, AlertTriangle, Clock, Building2, Receipt, Users, HardDrive, RefreshCw } from 'lucide-react';
...
const LINE_COLS = [
    ...
    { header: 'Audit Status', render: (r) => <Badge tone={STATUS_MAP[r.status]?.tone}>{STATUS_MAP[r.status]?.label}</Badge>, exportValue: (r) => STATUS_MAP[r.status]?.label },
  ...
  const JOB_COLS = [
    ...
    { header: 'Transactions Credit', accessor: 'recovered', align: 'right', render: (r) => inr(r.recovered), exportValue: (r) => r.recovered },
    { header: 'Variance', align: 'right', render: (r) => { const v = r.recovered - r.incurred; return <span className={v < 0 ? 'text-red-600 font-medium' : v > 0 ? 'text-amber-600' : ''}>{inr(v)}</span>; }, exportValue: (r) => r.recovered - r.incurred },
    { header: 'Line Items', accessor: 'count', align: 'right', exportValue: (r) => r.count },
    { header: 'Audit Status', render: (r) => <Badge tone={STATUS_MAP[r.status]?.tone}>{STATUS_MAP[r.status]?.label}</Badge>, exportValue: (r) => STATUS_MAP[r.status]?.label },
    { header: 'Days', align: 'right', render: (r) => daysSince(r.date), exportValue: (r) => daysSince(r.date) },
  ];

  const CUSTOMER_COLS = [
    { header: 'Customer', accessor: 'customer', exportValue: (r) => r.customer },
    { header: 'Transactions Debit', accessor: 'incurred', align: 'right', render: (r) => inr(r.incurred), exportValue: (r) => r.incurred },
    { header: 'Transactions Credit', accessor: 'recovered', align: 'right', render: (r) => inr(r.recovered), exportValue: (r) => r.recovered },
    { header: 'Unrecovered', accessor: 'unrecovered', align: 'right', render: (r) => inr(r.unrecovered), exportValue: (r) => r.unrecovered },
  ];

  const LEAK_COLS = [
    { header: 'Expense Head', accessor: 'head', exportValue: (r) => r.head },
    { header: 'Leaked Value', accessor: 'leaked', align: 'right', render: (r) => inr(r.leaked), exportValue: (r) => r.leaked },
    { header: 'Occurrences', accessor: 'count', align: 'right', exportValue: (r) => r.count },
  ];

  // Job-level status: worst-case rollup of a job's line items
  const jobStatus = (items) => {
    const has = (s) => items.some((r) => r.status === s);
    if (has('not-billed')) return 'not-billed';
    if (has('short')) return 'short';
    if (has('over')) return 'over';
    if (items.every((r) => r.status === 'pending')) return 'pending';
    return 'fully';
  };
  ...
  const jobWise = useMemo(() => {
    ...
    return Object.values(rows.reduce((acc, r) => {
      if (!acc[r.costCentre]) acc[r.costCentre] = { id: r.costCentre, costCentre: r.costCentre, customer: r.customer, incurred: 0, recovered: 0, count: 0, date: r.date, items: [] };
      acc[r.costCentre].incurred += r.incurred;
      acc[r.costCentre].recovered += r.recovered;
      acc[r.costCentre].count += 1;
      acc[r.costCentre].date = acc[r.costCentre].date < r.date ? r.date : acc[r.costCentre].date;
      acc[r.costCentre].items.push(r);
      return acc;
    }, {}))
    .map((j) => ({ ...j, status: jobStatus(j.items) }));
  }, [liveRows]);

  const filteredLines = tab === 'all' ? (liveRows || []) : (liveRows || []).filter((r) => r.status === tab);
  const filteredJobs = tab === 'all' ? jobWise : jobWise.filter((r) => r.status === tab);

  const customerWise = Object.values(
    filteredLines.reduce((acc, r) => {
      if (!acc[r.customer]) acc[r.customer] = { id: r.customer, customer: r.customer, incurred: 0, recovered: 0, unrecovered: 0 };
      acc[r.customer].incurred += r.incurred;
      acc[r.customer].recovered += r.recovered;
      acc[r.customer].unrecovered += Math.max(0, r.incurred - r.recovered);
      return acc;
    }, {})
  );

  const filterAppliedLines = tab === 'all' ? (liveRows || []) : (liveRows || []).filter((r) => r.status === tab);

  // Dynamic leak-by-head: computed from filtered data
  const leakByHead = useMemo(() => {
    const byHead = {};
    filterAppliedLines.forEach((r) => {
      const leaked = Math.max(0, r.incurred - r.recovered);
      if (!byHead[r.head]) byHead[r.head] = { head: r.head, leaked: 0, count: 0 };
      byHead[r.head].leaked += leaked;
      byHead[r.head].count += 1;
    });
    return Object.values(byHead).sort((a, b) => b.leaked - a.leaked);
  }, [filterAppliedLines]);
  ...
  return (
    <div className="space-y-6">
      ...
      {!liveRows ? (
        ...
        <>
      {/* Stat Grid — 5 cards, clickable to filter */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          sub={`${inrCompact(summary.notBilledValue)} in ${summary.notBilledCount} line items`}
          tone="amber"
          onClick={() => setTab('not-billed')}
        />
        <StatCard
      ...
      {/* View switcher — how the audit results are grouped */}
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[11px] font-medium text-ink-muted">View:</span>
        {[
          { id: 'jobs', label: 'Jobs', icon: Building2, tip: 'Summary grouped by job / cost centre' },
          { id: 'lines', label: 'Expenses', icon: Receipt, tip: 'Line-by-line expense detail' },
          { id: 'customers', label: 'Customers', icon: Users, tip: 'Summary grouped by customer' },
          { id: 'heads', label: 'Leakage', icon: AlertTriangle, tip: 'Expense heads ranked by unrecovered amount' },
        ].map((v) => {
          const Icon = v.icon;
          const active = viewType === v.id;
          return (
            <button
              key={v.id}
              ...
              title={v.tip}
              aria-pressed={active}
              className={`h-9 px-3 rounded-full text-xs font-medium transition-colors whitespace-nowrap inline-flex items-center gap-1.5 border ${
                active
                  ? 'bg-brand-600 text-white border-brand-600 shadow-soft'
                  : 'bg-white text-ink-muted border-line hover:bg-canvas-soft hover:text-ink'
              }`}
            >
      ...
      {loading ? (
        ...
        <>
          {viewType === 'jobs' && (
            <>
              ...
              <DataTable
                columns={JOB_COLS}
                data={filteredJobs}
                onRowClick={(job) => setDrill({ ...job, kind: 'job' })}
                exportFilename="reimbursement-audit-per-job.csv"
                paginate
                emptyMessage="No jobs in this bucket."
              />
            </>
          )}
          {viewType === 'lines' && (
          ...
          {viewType === 'customers' && (
            <DataTable columns={CUSTOMER_COLS} data={customerWise} exportFilename="reimbursement-audit-customers.csv" emptyMessage="No customer data." />
          )}
          {viewType === 'heads' && (
            <DataTable columns={LEAK_COLS} data={leakByHead} exportFilename="reimbursement-audit-leak-by-head.csv" emptyMessage="No leak data." />
          )}
        </>
      )}
      ...
      {/* Drill-down modal */}
      <Modal open={!!drill} onClose={() => setDrill(null)} title={drill?.kind === 'job' ? `Job: ${drill?.costCentre || ''}` : `Voucher Detail: ${drill?.costCentre || ''}`}>
        {drill?.kind === 'job' ? (
          <div className="space-y-3">
            ...
            <div className="pt-3 border-t border-line">
              <h4 className="text-sm font-semibold text-ink mb-2">Line Items ({drill.count})</h4>
              <DataTable
                columns={LINE_COLS}
                data={drill.items}
                exportFilename={`${drill.costCentre}-line-items.csv`}
